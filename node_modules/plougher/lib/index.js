...
Plougher.prototype.scrape = function(htmlOrUrl, config, done) {
    ...
    if (!htmlOrUrl) {
        throw new Error('First parameter must be a valid html or a url');
    }
    
    if (!config) {
		throw new Error('The second parameter should be a configuration object.');
	}

    if (!done) {
        throw new Error('Scrape function must be provided with a callback.');
    }

    //validate the config
    if (!Validator.isJSON(jsonString)) {
        throw new Error('Config should be a valid JSON format.');
    }
    ...
    Utils.getHtml(htmlOrUrl, function(err, html) {
        ...
        if (!html) {
            throw new Error('The url ' + htmlOrUrl + ' cound not be followed, please make sure its valid.');
        }

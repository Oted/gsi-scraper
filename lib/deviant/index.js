...
module.exports = function(done) {
    ...
    Async.eachLimit(internals.mapping.urls, 1, function(url, next) {
        ...
        return Request(options, function(err, httpResponse, body) {
            ...
            $('div.page-results > span').each(function(i, item) {
                var newObj = {
                    "title" : $(this).find('span.title').text().trim(),
                    "data" : $(this).attr('data-super-img'),
                    "source_type" : "deviant",
                    "source" : $(this).find('a.torpedo-thumb-link').attr('href'),
                    "category" : "creative",

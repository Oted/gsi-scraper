    ...
    Async.eachLimit(internals.mapping.urls, 1, function(url, next) {
        ...
        return Request(options, function(err, httpResponse, body) {
            ...
            $('div.page-results > div[userid]').each(function(i, item) {
                var newObj = {
                    "title" : $(this).find('a.t span').text().trim(),
                    "data" : $(this).find('a.thumb').attr('data-super-img'),
                    "source_type" : "deviant",
                    "source" : 'http://' + $(this).attr('username') + '.deviantart.com/',
                    "author" : $(this).attr('username'),
                    "category" : "creative",
                    "height" : $(this).find('a.thumb').attr('data-super-height')
                }

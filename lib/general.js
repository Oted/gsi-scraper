...
module.exports.scrapeImgur = function(link, done) {
    ...
    return Request(options, function(err, res, body) {
        ...
        if (scriptBody && scriptBody.height > 0) {
            newObj.height = scriptBody.height;
 ...
 * Vine
 */
module.exports.scrapeVine = function(link, done) {
    var options = {
        "timeout"   : 10000,
        "url" : link,
        'content-type' : 'application/json',
    ...
    return Request(options, function(err, res, body) {
        ...
        var script = $('script').map(function(i, s) {
            ...
            return s.indexOf("@context\":") > -1;
        });

        try {
            scriptBody = JSON.parse(script[0]);
        } catch (err) {
            return done(new Error('Could not scrape : ', link));
        }

        var newObj = {
            "title" : scriptBody.articleBody,
            "data" : scriptBody.image.split('.mp4').shift() + '.mp4',
            "source_type" : "vine",
            "type" : "video",
            "source" : options.url
        };

...
var DebugCracked        = require('debug')('cracked');
var DebugAngryDuck      = require('debug')('angryduck');
var DebugGiphy          = require('debug')('giphy');
var DebugSoundcloud     = require('debug')('soundcloud');
var DebugTwitch         = require('debug')('twitch');
var DebugPopkey         = require('debug')('popkey');
var DebugCheezburger    = require('debug')('cheezburger');
...
var DebugFunnyJunk      = require('debug')('funnyjunk');
var DebugInstagram      = require('debug')('instagram');
...
Scraper.prototype.scrape = function(file, mapping, done) {
    ...
    //if its popkey 
    if (file.indexOf('popkey') > -1) {
        return setTimeout(function() {
            return internals.scrapePopkey(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }
    ...
    //if its cracked
    if (file.indexOf('cracked') > -1) {
        return setTimeout(function() {
            return internals.scrapeCracked(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }

    //if its angry duck
    if (file.indexOf('angryduck') > -1) {
        return setTimeout(function() {
            return internals.scrapeAngryDuck(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }

    //if its cheezburger
    if (file.indexOf('cheezburger') > -1) {
        return setTimeout(function() {
            return internals.scrapeCheezburger(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }

    //if its funny junk
    if (file.indexOf('funnyjunk') > -1) {
        return setTimeout(function() {
            return internals.scrapeFunnyJunk(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }

    //if its instagram 
    if (file.indexOf('instagram') > -1) {
        return setTimeout(function() {
            return internals.scrapeInstagram(mapping, done);
        }, Math.floor(Math.random() * requestSpan));
    }
...
internals.requestAnimated = function(mapping, done) {
    ...
    Async.eachLimit(mapping.urls, 10, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            ...
            if (httpResponse.statusCode !== 200) {
                DebugAnimatedTabs('err in animated!', httpResponse.statusCode);
                return next();
            }
            ...
            try {
                ...
                DebugAnimatedTabs('Cound not parse animated tabs');
                return next();
            }
...
internals.requestTed = function(mapping, done) {
    ...
    Async.eachLimit(mapping.urls, 1, function(url, next) {
        ...
        Request.get(option, function(err, httpResponse, body) {
            ...
            if (httpResponse.statusCode !== 200) {
                DebugTed('err in ted!', httpResponse.statusCode);
                return next();
            }
    ...
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            ...
            $('div.post').each(function(i, item) {
                var newObj = {
                    "title" : ($(this).find('a[title]').attr('title') || "").trim().unleakString() + '[reaction]',
    ...
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            ...
            Async.eachLimit($('article.module'), 1, function(target, inext) {
                var newObj = {
                    "title" : $(target).find('h2 a').attr("title").trim().unleakString() + ' [documentary]',
 ...
 */
internals.scrapeInstagram = function(mapping, done) {
    var allItems = [];
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        var option = {
            "timeout"   : 10000,
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugInstagram('err in instagram!' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugInstagram('err in funny instagram!' + httpResponse.statusCode);
                return next();
            }
            ...
            if (!target) {
                DebugInstagram('Err could not get target in INSTAGRAM');
                return next();
            }
            ...
            try {
                ...
                DebugInstagram('could not fix instagram');
                return next();
            }
            ...
            targetObj.entry_data.ProfilePage.map(function(page) {
                page.user.media.nodes.map(function(item) {
                    var newObj = {
                        "title" : (item.caption || '').length >= 240 ? item.caption.slice(0,137) + '...' : item.caption,
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("instagram", err);
            return done();
        }

        DebugFunnyJunk('Got items ' + allItems);
        return done(null, allItems);
    });
};

/**
 *  Scrape the angry fucking duck!
 */
internals.scrapeAngryDuck = function(mapping, done) {
    var allItems = [];
    ...
    Async.eachLimit(mapping.urls, 1, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugAngryDuck('err in duck! ' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugAngryDuck('err in duck! ' + httpResponse.statusCode);
                return next();
            }
            ...
            Async.eachLimit($('div.thumbs a.thumbnail'), 3, function(target, iNext) {
                ...
                Request(innerOption, function(ierr, ihttpResponse, ibody) {
                    if (ierr) {
                        DebugAngryDuck('err in duck! ' + ierr);
                        return iNext();
                    }
                    
                    if (ihttpResponse.statusCode !== 200) {
                        DebugAngryDuck('err in duck! ' + ihttpResponse.statusCode);
                        return iNext();
                    }
                    ...
                    if (newObj.title && newObj.data) {
                        newObj.data = 'http://www.angryduck.com' + newObj.data.replace('.mp4', '.gif');
                        allItems.push(newObj);
                    }
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("angryduck", err);
            return done();
        }

        DebugAngryDuck('Got items ' + allItems);
        return done(null, allItems);
    });
};

/**
 *  Scrape the cheezburger!
 */
internals.scrapeCheezburger = function(mapping, done) {
    var allItems = [];
    ...
    Async.eachLimit(mapping.urls, 1, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugCheezburger('err in cheezburger! ' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugCheezburger('err in cheez! ' + httpResponse.statusCode);
                return next();
            }
            ...
            Async.eachLimit($('div[class*=col-] > a[href*="cheezburger.com"]'), 3, function(target, iNext) {
                ...
                Request(innerOption, function(ierr, ihttpResponse, ibody) {
                    ...
                    if (ihttpResponse.statusCode !== 200) {
                        DebugCheezburger('err in duck! ' + ihttpResponse.statusCode);
                        return iNext();
                    }
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("cheezburger", err);
            return done();
        }

        DebugCheezburger('Got items ' + allItems.length);
        return done(null, allItems);
    });
};
...
/**
 *  Scrape the cracked.com!
 */
internals.scrapeCracked = function(mapping, done) {
    var allItems = [];
    ...
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugCracked('err in cracked! ' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugCracked('err in cracked! ' + httpResponse.statusCode);
                return next();
            }
            
            var $ = Cheerio.load(body);
            
            Async.eachLimit($('div.meta a[href*=video_]'), 1, function(target, inext) {
                var innerOption = {
                    "timeout"   : 10000,
                    "uri"       : $(target).attr("href"),
                    "headers"   : {
                ...
                Request(innerOption, function(ierr, ihttpResponse, ibody) {
                    if (ierr) {
                        DebugCracked('err in cracked! ' + ierr);
                        return inext();
                    }
                    
                    if (ihttpResponse.statusCode !== 200) {
                        DebugCracked('err in cracked! ' + ihttpResponse.statusCode);
                        return inext();
                    }
                    ...
                    var i$ = Cheerio.load(ibody),
                        newObj = {
                            ...
                            "title" : i$('h1.title').text().trim().unleakString(),
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("cracked", err);
            return done();
        }

        DebugCracked('Got items ' + allItems);
        return done(null, allItems);
    });
};
...
/**
 *  Scrape the funny junk!
 */
internals.scrapeFunnyJunk = function(mapping, done) {
    var allItems = [];
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        var option = {
            "timeout"   : 20000,
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugFunnyJunk('err in funnyjunk!' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugFunnyJunk('err in funny junk!' + httpResponse.statusCode);
                return next();
            }
            
            var $ = Cheerio.load(body);
            
            Async.eachLimit($('ul.iThCompressed > li[class*=bg]'), 2, function(target, inext) {
                var innerOption = {
                    "timeout"   : 10000,
                ...
                Request(innerOption, function(ierr, ihttpResponse, ibody) {
                    if (ierr) {
                        DebugFunnyJunk('err in funny junk!' + ierr);
                        return inext();
                    }
                    
                    if (ihttpResponse.statusCode !== 200) {
                        DebugFunnyJunk('err in funny junk!' + ihttpResponse.statusCode);
                        return inext();
                    }
                    ...
                    var i$ = Cheerio.load(ibody),
                        newObj = {
                            "source" : innerOption.uri,
                            "title" : i$('h1.contentTitle').text().trim().unleakString(),
                            "data" :  
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("funnyjunk", err);
            return done();
        }
 ...
 *  Scrape the popkey!
 */
internals.scrapePopkey = function(mapping, done) {
    var allItems = [];
    Async.eachLimit(mapping.urls, 2, function(url, next) {
        var option = {
            "timeout"   : 10000,
        ...
        Request(option, function(err, httpResponse, body) {
            if (err) {
                DebugPopkey('err in popkey!' + err);
                return next();
            }
            
            if (httpResponse.statusCode !== 200) {
                DebugPopkey('err in popkey!' + httpResponse.statusCode);
                return next();
            }

            var $ = Cheerio.load(body);
            
            $('ul.tiles > li.tiles__item').each(function(i, item) {
                var newObj = {
                    "title" : $(this).find('a > img.tiles__media').attr('alt').replace('animated GIF', '').trim().unleakString(),
                    "data" :  $(this).find('a > img.tiles__media').attr('data-animated').split('?').shift(),
        ...
        if (err || !allItems || allItems.length < 1) {
            Utils.dealWithErrors("popkey", err);
            return done();
        }

        DebugPopkey('Got items ' + allItems);
        return done(null, allItems);
    });
};
